...
namespace Microsoft.Teams.Assist.Infrastructure.Logging.Serilog;
...
public static class Extensions
{
    ...
    private static void ConfigureElasticSearch(WebApplicationBuilder builder, LoggerConfiguration serilogConfig, string appName, string elasticSearchUrl)
    {
        if (!string.IsNullOrEmpty(elasticSearchUrl))
        {
            ...
            string indexFormat = $"{formattedAppName}-logs-{builder.Environment.EnvironmentName?.ToLower().Replace(".", "-")}-{DateTimeService2.UtcNow:yyyy-MM}";
